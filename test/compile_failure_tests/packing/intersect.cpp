...
int main()
{
    ...
    std::array<hit_record<basic_ray<float>, primitive<unsigned>>, 4> hr_array = simd::unpack(hr);

#elif defined HIT_RECORD_UNPACK_FLOAT8

#if VSNRAY_SIMD_ISA_GE(VSNRAY_SIMD_ISA_AVX)
    hit_record<basic_ray<simd::float8>, primitive<unsigned>> hr;
    std::array<hit_record<basic_ray<float>, primitive<unsigned>>, 8> hr_array = simd::unpack(hr);
#endif
