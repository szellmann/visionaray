...
template <typename P, typename N, typename M, typename L, typename C, typename ...Args>
struct kernel_params<P, N, M, L, C, Args...>
{
...
template <typename P, typename N, typename TC, typename M, typename T, typename L, typename C, typename ...Args>
struct kernel_params<P, N, TC, M, T, L, C, Args...>
{
    typedef P  primitive_type;
    typedef N  normal_type;
...
kernel_params<P, N, M, L, vec4>  make_params(P const& begin, P const& end, N const& normals,
...
{
    return kernel_params<P, N, M, L, vec4>
    {
...
kernel_params<P, N, TC, M, T, L, vec4> make_params(P const& begin, P const& end, N const& normals,
    TC const& tex_coords, M const& materials, T const& textures, L const& lbegin, L const& lend,
    vec4 const& bg_color = vec4(0.0, 0.0, 0.0, 1.0),
    vec4 const& ambient_color = vec4(1.0, 1.0, 1.0, 1.0))
{
    return kernel_params<P, N, TC, M, T, L, vec4>
    {
